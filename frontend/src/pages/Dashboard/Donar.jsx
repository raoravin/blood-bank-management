...
const Donar = () => {
  ...
  const getDonars = async () => {
    try {
      ...
      const {data} = await axios.get("http://localhost:8080/api/v1/inventory/get-donar",config)
     setData(data?.donar)
    } catch (error) {
      console.log(error)
    }
  ...
  return (
    <Layout>
      ...
      <tbody>
      <tr className=''>
          <th className='ps-5 p-3' scope="col">Name</th>
          <th className='ps-5 p-3' scope="col">email</th>
          <th className='ps-5 p-3' scope="col">phone</th>
          <th className='ps-5 p-3' scope="col">Time & Date</th>
        </tr>
      {data && data.map((item) => (
          <tr key={item._id} className='text-center'>
          <td className='ps-5 p-3' >{item.fullName || item.organisationName + "(ORG)"}</td>
